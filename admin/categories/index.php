            ...
            <table class="table table-hover table-striped table-bordered" id="list">
                ...
                <tbody>
                    ...
                    while($row = $qry->fetch_assoc()):
                        // Check if there are items under this category that are available
                        $itemQry = $conn->query("SELECT COUNT(*) AS available_count FROM `item_list` WHERE category_id = '{$row['id']}' AND status = '1'");
                        $itemStatus = ($itemQry->num_rows > 0) ? ($itemQry->fetch_assoc()['available_count'] >= 1 ? 1 : 0) : 0;
						$conn->query("UPDATE `category_list` SET `status` = '{$itemStatus}' WHERE `id` = '{$row['id']}'");
                    ?>
                        <tr>
...
<script>
    $(document).ready(function(){
        ...
        $('.edit-data').click(function(){
            uni_modal("<i class='fa fa-edit'></i> Edit New Category ","categories/manage_category.php?id="+$(this).attr('data-id'))
        })
