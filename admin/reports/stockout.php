        ...
        <div class="card rounded-0 mb-2 shadow">
            <div class="card-body">
                ...
                <div class="table-responsive">
                    <table class="table table-striped" id="stockout-table">
                        <thead>
                            <tr>
                                <th>Date</th>
                                <th>Category</th>
...
<script>
    ...
    function updateTotalQuantity() {
        var currentTotal = 0;
        $('#stockout-table tbody tr').each(function () {
            var quantity = parseInt($(this).find('td:last').text());
            if (!isNaN(quantity)) {
                currentTotal += quantity;
            }
        });
        $('#stockout-table tfoot tr:last-child th:last-child').text(currentTotal);
    }
    ...
    $('#filter-form').submit(function (e) {
        ...
        window.location = './?page=reports/stockout&' + formData;
    });
    ...
    $('#item').change(function () {
        // Perform an AJAX request to fetch the filtered results
        $.ajax({
            url: './?page=reports/stockout',
            method: 'GET',
            data: $('#filter-form').serialize(),
            success: function (response) {
                // Extract the table body content from the response
                var tableBody = $(response).find('#stockout-table tbody').html();

                // Update the table body with the filtered results
                $('#stockout-table tbody').html(tableBody);
